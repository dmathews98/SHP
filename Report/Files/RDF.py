...
class RDF(object):
    ...
    def parsL(self):
        ...
        self.vol = np.abs(np.linalg.det(self.L))
    ...
    def closest_z(self, step, i, j):
        closest_z_displ = np.mod((self.posarray[step][i][2]-self.posarray[step][j][2]+1.5), 1.0) - 0.5
    ...
    def RDF_norm(self, dr, numatoms, rdf, numruns):
        rho = (float(numatoms)*(numatoms - 1)/2.0)/self.vol
        # Density of ion pairs
        for i in range(len(rdf)):
            rsq = ((i+1)*dr)**(2.0)
            const = numruns * 4.0 * m.pi * dr * rho * rsq
            # Expected number of atoms at r
            rdf[i] = rdf[i]/const   # Scaling recorded number
    ...
    def find_RDF(self, atoms, rdf, numbins, dr, numatoms):
        # over all steps
        for step in range(1, len(self.posarray)):
            #for step % certain number -> not every step finding RDF
            if ((step%self.check) == 0):
                # atom 1 - eg all
                for i in range(0, atoms[2]):
                    # atom 2 - eg all
                    for j in range(i+1, atoms[2]):
                        closest_xy_displ_sq = self.closest_inplane(self.posxy[step][i], self.posxy[step][j]) # xy closest displ squared
                        closest_z_displ_sq = self.closest_z(step, i, j) # z closest displ squared
...
def main():
    ...
    numatoms = 160
    maxr = 8.0   # based on max sqrt(MSD) then just a little over
    dr = 0.02
    numbins = int(maxr/dr)
    ...
    RDF2 = RDF(FileName2, check, 2, initpos)
    RDF2.parsL()
    # unpos = RDF2.parsInitPos()
    RDF2.parsPos()

    RDF3 = RDF(FileName3, check, 3, initpos)
    RDF3.parsL()
    # unpos = RDF3.parsInitPos()
    RDF3.parsPos()

    rdf1 = RDF1.find_RDF(atoms, rdf, numbins, dr, numatoms)
    rdf2 = RDF2.find_RDF(atoms, rdf1, numbins, dr, numatoms)
    rdf3 = RDF3.find_RDF(atoms, rdf2, numbins, dr, numatoms)

    rdf_final = RDF3.RDF_norm(dr, numatoms, rdf3, numruns)
    x = np.arange(0, maxr, dr)

    # Plotting

    plt.plot(x, rdf_final)
    plt.xlabel(r'Distance between atoms ($\AA$)')
    plt.ylabel('RDF')
    plt.title('RDF for p3m1 for 300kbar and 1800K')
    plt.show()
