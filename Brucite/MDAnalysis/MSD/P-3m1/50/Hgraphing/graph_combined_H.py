...
def main():
    ...
    filet300 = open('p3m1_msd_H_p50_t300.txt', 'r')
    for line in filet300:
        t300.append(float(line))
    filet300.close()

    filet600 = open('p3m1_msd_H_p50_t600.txt', 'r')
    for line in filet600:
        t600.append(float(line))
    filet600.close()

    filet900 = open('p3m1_msd_H_p50_t900.txt', 'r')
    for line in filet900:
        t900.append(float(line))
    filet900.close()

    filet1200 = open('p3m1_msd_H_p50_t1200.txt', 'r')
    for line in filet1200:
        t1200.append(float(line))
    filet1200.close()

    filet1500 = open('p3m1_msd_H_p50_t1500.txt', 'r')
    for line in filet1500:
        t1500.append(float(line))
    filet1500.close()

    filet1800 = open('p3m1_msd_H_p50_t1800.txt', 'r')
    ...
    plt.title('MSDs for p-3m1 for Hydogen at different temperatures and 50kbar')
    plt.legend(('300K', '600K', '900K', '1200K', '1500K', '1800K'), loc='upper left')
    plt.savefig('p3m1_msd_H_p50.png')
    plt.show()
